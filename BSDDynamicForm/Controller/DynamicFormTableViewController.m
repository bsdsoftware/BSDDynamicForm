...
- (void)viewDidLoad {
	...
	[self.array addObject:modelConferma];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
	FieldModel *model = self.array[indexPath.row];
	switch (model.type) {
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
	if([segue.identifier isEqualToString:@"segueMultipleChoice"]) {
		...
		FieldModel *fieldSelected = self.array[selectedIndex.row];
...
- (IBAction)conferma:(id)sender {
	...
	[self.delegate onFormConfirm:self.array];
}
